...
export async function handleInteraction(interaction) {
  ...
  if (customId.startsWith('bau_')) {
    ...
    const checkChestPermission = (chest, action) => {
      if (!chest) return false;
      if (interaction.member.permissions.has(PermissionFlagsBits.Administrator)) return true;
      
      const allowedRoles = action === 'adicionar' 
        ? (chest.cargosAdicionarIds || chest.cargosPermitidosIds || [])
        : (chest.cargosRetirarIds || chest.cargosPermitidosIds || []);
        
      if (allowedRoles.length === 0) return false; // Sem cargos e não é admin -> Sem acesso
    ...
    if (interaction.isButton()) {
      try {
        ...
        if (customId === 'bau_adicionar_btn') {
          if (!checkChestPermission(chest, 'adicionar')) {
            return await interaction.reply({
              content: '❌ Você não tem cargo autorizado para depositar (adicionar) itens neste baú!',
              ephemeral: true
            });
          }

          const globalItems = getBauItems();
          const specificItems = chest.itensExclusivos || [];
          const materials = [...new Set([...globalItems, ...specificItems])];
          
          if (materials.length === 0) {
            return await interaction.reply({
              content: '❌ Nenhum item cadastrado no sistema para adicionar.',
              ephemeral: true
            });
          }
        ...
        if (customId === 'bau_retirar_btn') {
          if (!checkChestPermission(chest, 'retirar')) {
            return await interaction.reply({
              content: '❌ Você não tem cargo autorizado para sacar (retirar) itens deste baú!',
              ephemeral: true
            });
          }
    ...
    if (interaction.isStringSelectMenu()) {
      try {
        ...
        if (customId.startsWith('bau_adicionar_select_')) {
          ...
          if (!checkChestPermission(chest, 'adicionar')) {
            return await interaction.reply({ content: '❌ Permissão de depósito negada!', ephemeral: true });
          }
        ...
        if (customId.startsWith('bau_retirar_select_')) {
          ...
          const chest = getBau(chestMessageId);
          if (!checkChestPermission(chest, 'retirar')) {
            return await interaction.reply({ content: '❌ Permissão de retirada negada!', ephemeral: true });
          }
    ...
    if (interaction.isModalSubmit()) {
      try {
        ...
        if (customId.startsWith('bau_adicionar_modal_')) {
          ...
          if (!checkChestPermission(chest, 'adicionar')) {
            return await interaction.reply({ content: '❌ Permissão de depósito expirada ou inválida!', ephemeral: true });
          }
        ...
        if (customId.startsWith('bau_retirar_modal_')) {
          ...
          if (!checkChestPermission(chest, 'retirar')) {
            return await interaction.reply({ content: '❌ Permissão de retirada expirada ou inválida!', ephemeral: true });
          }
